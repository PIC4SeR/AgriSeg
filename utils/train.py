...
from utils.isw import CovMatrix_ISW, instance_whitening_loss
...
class Trainer:  
    ...
    def get_data(self, test_only=False):
        ...
        if self.ds_train is not None:
            ...
            if self.config['STYLE_AUG']:
                self.ds_train = self.ds_train.map(lambda x, y: random_jitter(x, y, p=self.config['RND_JITTER'], 
                                                  r=self.config['RND_JITTER_RNG'], seed=self.seed),
                                                  tf.data.experimental.AUTOTUNE)
        ...
        if self.ds_val is not None:
            ...
            self.ds_val = self.ds_val.batch(self.config['BATCH_SIZE'], drop_remainder=True)
        ...
        if self.ds_test is not None:
            ...
            self.ds_test = self.ds_test.batch(self.config['BATCH_SIZE'], drop_remainder=True)
    ...
    def get_model(self):    
        ...
        self.model = build_model_binary(pre_trained_model, False, self.config['N_CLASSES'], 
    ...
    def get_opt_loss(self):
        if self.config['OPTIMIZER'] == 'adamw':
            lr_sched = tf.keras.optimizers.schedules.PolynomialDecay(initial_learning_rate=float(
                self.config['ADAMW']['LR']), 
                                                                     decay_steps=self.n_steps, 
                                                                     end_learning_rate=float(
                                                                         self.config['ADAMW']['LR_END']), 
            ...
            lr_sched = tf.keras.optimizers.schedules.ExponentialDecay(float(self.config['SGD']['LR']),
                                                                      decay_steps=self.n_steps,
