...
class Trainer:  
    ...
    def get_model(self):    
        ...
        whiten_layers = self.cfg['WHITEN_LAYERS'] if self.cfg['UNISTYLE'] \
                        and self.cfg['METHOD'] in ['ISW','XDED','IBN','KD'] else [],
    ...
    def get_opt_loss(self):
        ...
        if self.cfg['METHOD'] == 'XDED':
            self.xded = pixelwise_XDEDLoss(temp_factor=self.cfg['KD']['T'])
